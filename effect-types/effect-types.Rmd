...
Si le traitement est assigné de manière aléatoire, $D_{i}$ est indépendant, non seulement des résultats potentiels mais aussi de toutes les covariables (observées et non observées) qui pourraient également prédire ces résultats $((Y_{i}(1),Y_{i }(0),X_{i}⊥⊥D_{i}))$.[^2]
...
Supposons que nous devions réassigner à plusieurs reprises le traitement de manière aléatoire et à chaque fois calculer la différence des moyennes entre les groupes traités et témoins, puis enregistrer cette valeur dans une liste.
...
Lorsque notre traitement affecte à la fois la valeur moyenne du résultat et la distribution des réponses, cette hypothèse ne tient plus et nous devons ajuster l'erreur standard de MCO à l'aide d'un estimateur sandwich Huber-White pour obtenir les estimations correctes (basées sur la variance de l'ATE) pour l'inférence statistique.[^6]
...
Cela s'avère être un problème difficile (pour en savoir plus à ce sujet, consultez ce guide).
...
Calculer l'erreur standard pour les ratios est quelque peu fastidieux et nous estimons donc généralement un CACE en utilisant une régression des moindres carrés en deux étapes avec assignation aléatoire, $Z_i$, servant d'instrument pour la réception du traitement $D_i$ dans la première étape du modèle.
Cette approche simplifie l'estimation de l'erreur standard et permet l'inclusion de covariables en tant qu'instruments supplémentaires.
Dans le code ci-dessous, nous présentons les deux stratégies pour les données présentant une non-conformité bilatérale.
Notez, cependant, que lorsque les instruments ne sont pas puissants (par exemple, l'assignation aléatoire n'a eu qu'un faible effet sur la réception du traitement), les estimateurs des variables instrumentales et leurs erreurs standard peuvent être biaisés et incohérents.[^10]
...
5 Effet moyen du traitement sur une population ou un échantillon
==
Souvent, à partir de notre échantillon, nous voulons généraliser des déclarations pour une population d'intérêt plus large.[^11]
...
Dans la pratique, le pool expérimental peut consister en un groupe d'unités sélectionnées de manière inconnue à partir d'une population vaguement définie de ces unités et la conformité à l'assignation du traitement peut être moins que complète.
...
Par exemple, les personnes participant à un essai médical diffèrent souvent de la population pour laquelle le médicament serait disponible.
...
Bien qu'il n'y ait pas de solutions simples aux problèmes créés par une telle erreur, vous pouvez prendre des mesures à la fois dans le design de votre étude et dans l'analyse de vos données pour relever ces défis liés à l'estimation du PATE ou du CACE/LATE.
Par exemple, l'inclusion d'une intervention placebo fournit des informations supplémentaires sur la probabilité de recevoir un traitement, qui peuvent être utilisées pour repondérer l'effet de le recevoir réellement (par exemple Nickerson (2008)) en présence de non-conformité.
...
[^13]: Angrist et Pischke (2008) fournissent une brève introduction des sujets traités plus en détail par Hirano, Imbens et Ridder (2003), Aronow et Middleton (2013), Glynn et Quinn (2010), et Hartman et al. (à venir)

Dans le code ci-dessous, nous présentons plusieurs approches pour estimer ces effets mis en œuvre dans le package CausalGAM pour R.[^14]
...
Ci-dessous, nous créons une étude de cas où la probabilité de recevoir un traitement varie et peut être estimée à l'aide d'un modèle de score de propension.[^16]
...
La qualité (non biaisée) de ces estimations est intrinsèquement liée à la qualité de nos modèles pour prédire la réception du traitement.
...
7 effet moyen de traitement par quantile
==
...
Nous voulons savoir non seulement si notre traitement a augmenté le revenu moyen, mais aussi s'il a rendu la répartition des revenus dans l'étude plus ou moins égale.
...
Si ces hypothèses sont justifiées pour nos données, nous pouvons obtenir des estimations cohérentes de l'effet du traitement par quantile (QTE) en utilisant la régression par quantile.[^19]
...
Notez que l'interprétation du QTE est pour un quantile donné, pas un individu à ce quantile.
...
==
Parfois, nous voulons décrire non seulement l'ampleur et la significance d'un effet causal observé, mais aussi le mécanisme (ou les mécanismes) qui l'ont produit.
...
L'estimation impartiale de ces effets, cependant, nécessite un ensemble d'hypothèses fortes sur la relation entre le traitement, les médiateurs, les résultats et les facteurs de confusion potentiels, collectivement appelés ignorabilité séquentielle (Imai, Keele et Yamamoto (2010), Bullock, Green et Ha (2010 )).[^20]
...
Dans le code, nous montrons certaines des caractéristiques de leur approche, implémentée dans le package de médiation dans R (Tingley et al. 2014).
...
Dans nos données simulées, un peu plus de 20 % de l'effet total est médié par notre médiateur proposé, M et le biais d'un facteur de confusion pré-traitement non observé devrait être assez important (ρ = 0,7) avant que nous rejetions la conclusion de un ACME positif.
...
Aussi voir la discussion de Imai, Tingley et Yamamoto (2013) pour différentes perspectives quand aborder une affirmation sur la médiation avec une analyse de sensibilité ou une analyse des bornes de la sensibilité.
...
Une quantité commune d'intérêt causal pour les résultats dichotomiques est l'effet du traitement sur les log-odds de succès, défini pour le pool expérimental comme :

$$\Delta = log\frac{E(Y_i(1))}{1-E(Y_i(1))} - log\frac{E(Y_i(0))}{1-E(Y_i(0))}$$

Freedman (2008b) montre que l'ajustement de covariables par régression logistique dans les expériences randomisées produit des estimations biaisées de cet effet causal.
L'intuition de base de l'argument de Freedman vient du fait que prendre le log des moyennes n'est pas la même chose que prendre la moyenne des logs et donc le coefficient de traitement estimé à partir d'une régression logistique conditionnée sur les covariables ne fournira pas un estimateur cohérent des log-odds de succès.
...
Les coefficients de la régression logistique contrôlant la covariable X ont tendance à surestimer l'effet du traitement sur les probabilités logarithmiques, tandis que les estimations ajustées à partir des probabilités prédites produisent des résultats cohérents.
...
Considérons un cas simple avec un résultat et un traitement dichotomique.
Soit $A$ le nombre de résultats attribuables au traitement, c'est-à-dire le nombre de cas dans lesquels $Y_i$ était égal à 1 parmi les sujets traités ; ce qui ne se serait pas produit si ces unités avaient été assignées au contrôle.
Pour une plage de $A$, nous ajustons le tableau de contingence observée des résultats parmi les traités et comparons cette distribution résultante à une distribution nulle connue (la distribution des résultats que nous aurions observé si le traitement n'avait eu aucun effet).
La plage résultante de $A$ pour laquelle notre test continue de rejeter l'hypothèse nulle d'absence d'effet fournit une plage d'effets qui sont attribuables à notre traitement.
...
Rosenbaum (2002) extend ce concept à différents types de résultats (tels que les variables continues).
...
Hansen et Bowers (2009) utilisent cette approche pour identifier le nombre de votes supplémentaires attribuables à différentes interventions dans l'expérience de sortie du vote avec une assignation de traitement par grappe et une non-conformité unilatérale.
...
Deuxièmement, notez que si nous écrivons l'ensemble $U$ comme pool expérimental et que l'ensemble des unités de contrôle est un sous-ensemble de l'ensemble, $C⊆U$, alors nous pouvons écrire $∑_{i∈C}Y_i −Y_i(0)=0$.
...
Troisièmement, cette représentation nous permet de produire un intervalle de confiance basé sur un design pour A^ en s'appuyant sur la littérature sur l'échantillonnage pour les sondages basé sur l'inférence statistique pour les échantillons totaux, car les résultats totaux observés, tU, sont fixés à travers une randomisation.
...
Dans le code ci-dessous, nous fournissons une illustration utilisant des données simulées pour une réponse et un traitement binaires.
