...
Abstract
==
Researchers are rarely able to collect measurements on all units that make up the target population about which researchers would like to learn. Time and budget constraints typically require the selection of a subset of units -- a process called sampling. This guide provides an overview of different ways to sample and how these affect what can be learned from a study. Particular attention is paid to questions surrounding sampling for randomized experiments.

1.	What is random sampling?
==

Random sampling, also referred to as probability sampling, is one way of selecting a subset of elements from a target population. The key characteristic of random sampling is that each element in the population has a *known* and *non-zero* probability of being included in the sample. This goal is achieved by using some randomization mechanism -- be it a coin flip, a random number table, or a computer program -- to decide which elements of the population are going to be part of the sample. 
...
sampling_dist_SATE <- data.frame(unit_id = rep(NA, length(sampling_dist_SATE)),
                                 ...
                                 label = "Distribution of true SATEs across possible samples")
...
plot_data$label <- "Distribution of effect estimates across possible samples and assignments"

specific_sample <- plot_data %>% filter(sample == 10)

specific_sample$label = "Distribution of effect estimates across possible assignments in a given sample"
...
plot_data$label <- factor(plot_data$label, levels = c("Distribution of true SATEs across possible samples",
                                                      "Distribution of effect estimates across possible assignments in a given sample",
                                                      "Distribution of effect estimates across possible samples and assignments"))

data_summarized$label <- factor(data_summarized$label, levels = c("Distribution of true SATEs across possible samples",
                                                                  "Distribution of effect estimates across possible assignments in a given sample",
                                                                  "Distribution of effect estimates across possible samples and assignments"))
...
The first panel in the figure above plots the distribution of the true average effect of our treatment across the 15 possible samples of four units that we may draw. In constructing this distribution, we presume that we can observe *both* potential outcomes among four randomly sampled units. We thus suppress uncertainty that comes from the process of random assignment and focus on sampling uncertainty alone. Since we draw our samples randomly, the distribution of SATEs is centered on the true PATE of $-3$. Among any given sample, however, the average effect of the treatment may be larger or smaller than the PATE.
...
Since there are 15 possible samples and six ways of randomly assigning treatment within each sample, there are $15 \times 6 = 90$ possible estimates that may result in our hypothetical study. The last panel of the figure plots the distribution of all possible effect estimates, thus taking into account both uncertainty from sampling and from random assignment. 

If we would like to make inferences to population quantities, our uncertainty estimates should reflect both kinds of uncertainty. If we use randomization inference to calculate $p$-values, for example, we only consider variability that stems from the random assignment of units to treatment and not from the sampling of units itself. The standard Neyman estimator of the variance of the difference-in-means estimator, on the other hand, produces standard errors that are conservative if we presume that all uncertainty comes from random assignment. These standard errors will provide unbiased variance estimates if we can conceive of the experimental subject pool as a random sample from a population that is infinite. This assumption seems plausible when the experimental sample is small relative to the population. Where the size of the experimental sample is large relative to the population, Neyman standard errors will again be conservative. See @aronow2014sharp and @abadie2020sampling for more on how to obtain unbiased variance estimates when sampling from a finite population.  
...
Populations that are rare or stigmatized are difficult to study through random sampling. The inferential target in studies of difficult-to-reach population is often the prevalence of some characteristic among the target population or the size of the population itself. The rate of HIV infections among injecting drug users and the size of the population of sex workers are classic examples. Sampling frames are not generally available for such groups and a random sample of the general population is likely to contain an insufficient number of their members. Respondent-driven-sampling (RDS) has emerged as one prominent solution to this problem.^[See @lohr2009sampling[chapter 14] for an overview of other approaches and @tran2015sampling for a comparison of RDS to time-location sampling as a major alternative.] 

The core advantage of RDS is that it makes use of the social network ties that link members of the target population to facilitate sampling. The sampling process starts with an initial non-random sample. Respondents in this initial sample are given a fixed number of coupons which they can hand out to their contacts in the target population. In subsequent waves of sampling, respondents are recruited through the coupons that are given to them by respondents in the previous wave. Typically, respondents are compensated for every additional respondent whom they recruit. The idea is that the influence of the initial non-random sample can be reduced by conducting many successive waves of sampling. 

Nonetheless, RDS does not yield a true random sample, because the probability of being sampled for each member of the target population remains unknown. In practice, researchers use various models to approximate the sampling process when estimating the population parameters of interest. The simplest model conceives of the sampling process as a random walk, where each respondent, when selecting whom to recruit for the next wave, chooses at random among her contacts in the target population  [see @gile20107]. This model imples that each respondents unconditional probability of being sampled is proportional to her number of ties to the network that links the target population. This number can be elicited as part of the survey process and then used for weighting in the estimation process. 
