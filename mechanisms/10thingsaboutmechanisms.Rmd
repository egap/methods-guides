...
Cependant, nous définissons également deux résultats potentiels complexes de Y : le Y présenté par l'individu qui n'est pas traité mais qui révèle son résultat potentiel M traité, et le Y présenté par l'individu qui est traité mais qui révèle ses résultats potentiels M non traités.
Bien que ces résultats potentiels compliquent la compréhension, ils sont importants à définir dans l'hypothèse afin que nous puissions calculer les effets directs et indirects "vrais" (mais intrinsèquement inobservables) auxquels comparer notre analyse de décomposition.
...
# construire dans une caractéristique non observée idéosyncratique
Q_i <- rnorm(N)
...
tau_X_on_Y <- 0.5 # effet total de X sur Y (ATE), à travers M et non
...
# construire les résultats potentiels pour le résultat
...
# observrions si nous avions assigné au traitement mais observé le RP de M non traité ou
...
# nous révélons les RPs pour M et Y en fonction de l'Assignation du traitement
M <- X*M_1 + (1-X)*M_0
Y <- X*Y_1 + (1-X)*Y_0
...
# on pourrait multiplier maintenant l'effet moyen de X sur M et l'effet moyen de M sur Y pour obtenir l'ACME de X sur Y via M
a*b
...
# les estimations qui ne sont pas biaisées sont les effets moyens de X sur Y et de X sur M car X est assigné au hasard
a
ATE_M
c
ATE
...
Étant donné que M n'est pas attribué au hasard, il est important pour nous de réfléchir à la probabilité que notre M et notre Y soient tous deux affectés par des variables non observées.
En principe, s'il n'y a pas de variables de confusion dans cette relation, alors une analyse de décomposition des effets peut être impartiale, mais cette hypothèse est forte et généralement difficile à prouver.
...
Aux points 3-6, nous avons mis en garde les chercheurs contre toute tentative de quantifier avec confiance la proportion d'un effet de médiation par un mécanisme particulier,
...
Au lieu de cela, elle a recherché un effet du traitement hétérogène que la théorie aurait impliqué et, n'en trouvant aucune preuve, a conclu que le canal de revenu peut être moins important qu'on ne le pensait auparavant.
...
Au point 7, nous l'avons fait se demandant si le traitement a affecté des sous-groupes particuliers pour lesquels un effet de traitement est induit par notre théorie.
...
Premièrement, selon la théorie de la modernisation, l'éducation pourrait faciliter le bon fonctionnement de la démocratie en sapant les attachements au groupe (comme l'appartenance ethnique ou la religion) au profit du mérite.[^8] Deuxièmement, selon les théoriciens sociaux de l'oppression, l'éducation pourrait saper la démocratie en renforçant l'obéissance à l'autorité, qui est inhérente à une structure de classe.[^9] Troisièmement, selon de nombreux politologues et psychologues, l'éducation peut encourager la participation démocratique en donnant aux individus la capacité d'acquérir et d'agir sur la connaissance.[^10] Friedman et Al. (2011)[^11] décident de séparer ces mécanismes en enquêtant sur les résultats d'une expérience sur le terrain dans laquelle des filles kenyanes ont été assignées au hasard pour recevoir une subvention à l'éducation.
 Ils ont effectué un suivi auprès des élèves cinq ans après le programme et leur ont posé plusieurs questions visant à tester lequel de ces trois mécanismes était à l'œuvre : les filles ont-elles accepté le droit d'un mari de battre sa femme ? Un parent a-t-il participé au choix de leur mari? Dans quelle mesure la fille s'est-elle identifiée à son groupe religieux ou ethnique ? La fille lisait-elle régulièrement les actualités ?
...
Cependant, la théorie de la modernisation aurait prédit une diminution de l'association de groupes religieux ou ethniques (en réalité, il n'y avait aucun effet) et n'aurait eu aucune prédiction pour la lecture des journaux (en réalité, la lecture des journaux a augmenté).
...
S'ils soutiennent toujours que l'autonomisation individuelle est responsable de la relation entre l'éducation et la démocratie, ils avertissent que l'éducation ne conduit pas toujours à la démocratisation (c'est-à-dire M3→O mais il est également possible que M3→NON Y).
...
# Le design de traitements complexes peut aider à affiner notre compréhension de quelle est la partie du traitement qui "produit l'effet".
...
Cette approche est parfois appelée "analyse de médiation implicite" car différents composants X sont censés manipuler *implicitement* certains mécanismes.
...
Pour distinguer ces mécanismes, Baird et al. (2011)[^13] ont mené une expérience au Malawi, où ils ont assigné un groupe de familles pour recevoir un transfert en espèces *conditionnel* à la fréquentation scolaire régulière de leurs filles, un autre groupe de familles pour recevoir l'argent *sans condition*, et un groupe témoin pour ne recevoir aucun transfert.
Ce design manipulait "implicitement" M : alors que les filles du groupe de transfert inconditionnel pouvaient également poursuivre une éducation, la fréquentation scolaire (la condition à l'étude) serait probablement plus élevée dans le groupe qui devait la poursuivre.
...
**Ce que vous pouvez faire...** Dans les projets futurs, posez-vous la question : mon traitement peut-il être "décomposé" en plusieurs bras de traitement, certains qui manipulent implicitement M, et d'autres non ? Envisagez d'utiliser un plan factoriel pour identifier les effets des différents bras de traitement.
...
Pour ces raisons, le public (qu'il s'agisse du grand public ou des relecteurs universitaires) est souvent naturellement impatient que vous exposiez les mécanismes de causalité après avoir démontré des preuves d'une affirmation causale provocatrice.
En prévision de cela, il convient de se demander s'il est possible de concevoir un moyen de tester les mécanismes causaux avant de mettre en œuvre une expérience.
...
Les mécanismes sont un domaine d'investigation passionnant et doivent être pris en compte à la fois dans le design et l'analyse d'une expérience, mais nous devons nous assurer de discuter des mécanismes avec prudence, adaptés à notre capacité à identifier un mécanisme particulier et à éviter de surestimer l'argument.
