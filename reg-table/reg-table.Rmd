...
9 When a regression is to be believed
==
...
==
So far, this guide has referred to Ordinary Least Squares (OLS) regression, which is one of the most commonly used regression techniques in social sciences. In fact, there are many different types of regression that vary mostly in terms of the underlying equation that is being estimated. These include weighted least squares regressions and generalized least squares regressions, as well as all sorts of models for non-linear (curved) relationships or for situations where the outcome variable is restricted to take on particular values, such as being binary (0/1), categorical (A,B,C,…), count (0,1,2,…), and so on. 

* Authors might use a weighted least square regression when the variation of the observations are not equal (this is called heteroskedasticity, which generally looks something like this).
![](https://raw.githubusercontent.com/egap/methods-guides/master/reg-table/Rplot.income.cone.jpg)
* You might see a logit or a probit regression when the dependent variable is binary, meaning it has only two options: yes/no, 0/1, or True/False.  Logit and probit differ in terms of the assumptions about the underlying data generating process, but they tend to yield similar results and often deciding between the two is often a matter of author preference. 
* Ordinal logit and ordinal probit models are used when there are multiple ordered categories (such as “strongly disagree,” “disagree,” “agree,” “strongly agree”).
* Multinomial logit or multinomial probit models are used when there are multiple unordered categories (“Labour,” “Conservative,” “Lib Dem”).
* Poisson or Negative Binomial models when the outcome is a count (“how many riots this year”).
* Tobit models, when the dependent variable is non-negative (“How much time spent working this month”)
* and many more…

For the simple linear case, the coefficient tells you the change in Y you get for each unit change in X, but for non-linear regressions the interpretation can be much more difficult. For non-linear models you should generally expect authors to provide substantive interpretations of the coefficients. The program [Clarify](http://gking.harvard.edu/clarify) by Gary King and colleagues helps with this for Stata users; the [Zelig package for R](http://zeligproject.org/) (also by King and coauthors) supports analysis and interpretation of these models in R.
