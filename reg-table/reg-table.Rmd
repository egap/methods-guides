...
==
Everything in here is strictly speaking about "Ordinary Least Squares" (OLS) regression, which is one of the most commonly used regression techniques in social sciences, but in fact there are many different types of regression that vary mostly in terms of the underlying equation that is being estimated. These include weighted least squares regressions, generalized least squares regressions, as well as all sorts of models for non-linear (curved) relationships or for situations where the outcome variable is restricted to take on particular values, such as being binary (0/1), categorical (A,B,C,…), count (0,1,2,…) and so on. 

* Authors might use a weighted least square regression when the variation of the observations are not equal (this is called heteroskedasticity, which generally looks something like this- include a graph). 
* You might see a logit or a probit regression when the dependent variable is binary, meaning it has only two options: yes/no, 0/1, or True/False.  Logit and probit differe in terms of the assumptions about the underlying data generating process, but they tend to yield similar results and often deciding between the two is often a matter of author preference. 
