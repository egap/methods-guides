...
Un design de recherche expérimental devrait permettre à un chercheur d'estimer l'effet causal de $Z$ sur $Y$ sous des hypothèses standard.
...
```{r, echo = F, warning = F, message = F, fig.cap="Le processus de mesure dans le contexte d'un design de recherche expérimental."}
library(ggplot2)
...
# 2. La mesure est le lien entre l'argument substantif et/ou théorique d'un chercheur et un design de recherche (expérimental).

Lorsque nous considérons le design d'une expérience, nous avons tendance à nous concentrer sur le processus par lequel le traitement assigné de manière aléatoire, $Z$ est assigné et la distribution conjointe de $Z$ et d'un résultat, $Y$.
En d'autres termes, nous avons tendance à séparer les scores de $Z$ et $Y$ des concepts plus larges lorsque nous considérons les propriétés statistiques d'un design de recherche.
Dans ce récit, deux expériences complètement séparées avec la même distribution de $Z$ et $Y$ pourraient avoir des propriétés identiques.

Par exemple, un essai clinique sur l'efficacité de l'aspirine sur les maux de tête et une expérience qui fournit des informations sur le niveau de corruption d'un politicien sortant et demande ensuite à la personne interrogée si elle votera pour le candidat sortant *pourrait* avoir des échantillons, des assignations, des estimandes et réalisations des résultats (données).
Pourtant, caractériser comme "équivalents" ces deux projets de recherche complètement distincts qui cherchent à faire des inférences complètement distinctes peut nous sembler assez étrange ou même troublante.

Cependant, lorsque nous considérons la mesure comme une composante fondamentale du design de recherche, ces expériences sont clairement distinctes.
...
# 3. La mesure des traitements comprend l'opérationnalisation du traitement ainsi que le respect de l'assignation du traitement.
...
Pourtant, fournir des informations inclut également le mode de livraison et qui a fourni les informations.
...
Premièrement, en assignant un traitement, les expérimentateurs visent à contrôler les valeurs que prend une unité donnée.
...
Un sujet peut ou non avoir reçu le traitement, mais son score sur l'indicateur de traitement est simplement le traitement auquel elle a été assignée, et non le traitement qu'elle a reçu.
...
Pour ce faire, nous définissons ce qui constitue le respect de l'assignation de traitement.
...
Notre définition de la conformité devrait déterminer ce qui constitue un "traité" sur notre mesure (endogène) : "savoir si un ménage a reçu le traitement", ici l'information.
Certaines définitions courantes de la conformité peuvent être (a) qu'un membre du ménage a ouvert la porte ; ou (b) que quelqu'un du ménage a écouté le script d'information complet.
...
Les résultats, y compris les connaissances, les préférences et les attitudes, sont latents.
Nous enregistrons ou notons ainsi des indicateurs observables supposés être liés au résultat latent dans un effort pour déduire les caractéristiques de la variable latente.
...
Comme dans la section 1 ci-dessus, si ces indicateurs ne mesurent pas conceptuellement le concept d'intérêt, alors les inférences que nous faisons sur la relation entre $Z$ et $Y$ (même avec un design "parfait" en termes de puissance statistique et de données manquantes, etc.) peuvent ne pas nous renseigner sur "l'inférence ultime" que nous cherchons.
...
Au lieu de cela, nous demandons si les unités soutiennent divers comportements pensés pour correspondre à ces normes.
Cet indicateur, $Y_i$, peut être décomposé en la variable latente, $\nu_i$ et deux formes d'erreur de mesure :

- *Erreur de mesure non systématique*, $\delta_i$ : Cette erreur est indépendante de l'assignation du traitement, $\delta_i \perp Z_i$.
- *Erreur de mesure systématique*, $\kappa_i$ : Cette erreur n'est pas indépendante de l'assignation du traitement, $\kappa_i \not\perp Z_i$.
...
Tant que $Cov(\nu_i, \delta_i)\geq 0$ (nous supposons souvent que $Cov(\nu_i, \delta_i)= 0$), il doit être vrai que $Var(Y_i)$ augmente comme erreur de mesure, ou $Var(\delta_i)$ augmente.
...
Qu'en est-il du cas où $Cov(\nu_i, \delta_i) < 0$ ? Bien que cela réduise $Var(Y_i)$ (en maintenant $Var(\nu_i)$ et $Var(\delta_i)$ constants), cela atténue également la variation que nous mesurons dans $Y_i$.
En principe, cela devrait atténuer le numérateur $|\mu_t-\mu_c|$, qui réduira également la puissance, si cela est suffisant par rapport à la réduction de la variance.

# 7. L'erreur de mesure systématique biaise les estimations de l'effet causal d'intérêt.

Si nous estimons l'effet de traitement moyen (ATE) de notre traitement $Z_i$, sur les préférences pour les normes démocratiques, $\nu_i$, nous essayons de calculer $E[\nu_i|Z_i = 1] - E[\nu_i|Z_i =0]$.
Cependant, en présence d'une erreur de mesure **systématique**, où l'erreur de mesure est liée à l'assignation du traitement lui-même (disons, le résultat est mesuré différemment dans les groupe de traitement et de contrôle), un estimateur de la différence des moyennes sur le résultat observé, $Y_i$, renvoie une estimation biaisée de l'ATE.
L'effet du traitement comprend désormais la différence de mesure ainsi que la différence entre les groupe de traitement et de contrôle :
...
De plus, les designs qui mesurent les résultats de manière asymétrique dans les groupes de traitement et de contrôle peuvent être sujets à une erreur de mesure systématique.
Dans tous les cas, il existe une asymétrie entre les conditions de traitement dans : (a) la manière dont les sujets réagissent à l'observation ; ou (b) la façon dont nous observons les résultats qui sont distincts de tout effet du traitement sur la variable latente d'intérêt.
L'estimation biaisée de l'ATE devient net de tout effet sur les variables latentes (l'ATE) et l'erreur de mesure non systématique.
...
Au-delà de la considération de la qualité du mapping entre un concept et une mesure, nous pouvons souvent évaluer la qualité de la mesure en la comparant à des mesures provenant d'opérationnalisations alternatives du même concept, de concepts étroitement liés ou de concepts distincts.
...
Dans quelle mesure une mesure d'un concept latent prédit-elle un comportement que l'on pense être façonné par le concept ?
...
Les tests de *validité discriminante* examinent les indicateurs d'un concept et d'un concept apparenté mais distinct.
En principe, nous recherchons des corrélations faibles (corrélations proches de 0) entre les deux indicateurs.
Une limitation des tests de *validité discriminante* est que nous ne savons pas comment les concepts distincts sous-jacents varient.
...
["10 choses à savoir sur les comparaisons multiples"](https://egap.org/resource/10-things-to-know-about-multiple-comparisons) décrit des indices construits à partir de la cote-$z$ et d'une pondération inverse de covariance pour des résultats multiples.
...
Ci-dessous, nous examinons un simple indice de cote $z$ pour deux mesures bruitées d'une variable latente.
...
Premièrement, nous pouvons montrer qu'à travers de nombreuses simulations des données, la corrélation entre l'indice de cote $z$ des deux mesures et la variable latente est, en moyenne, plus élevée que la corrélation entre l'un ou l'autre des indicateurs et la variable latente.
Lors de la représentation graphique de la corrélation des mesures individuelles et de la variable latente par rapport à (sur l'axe des $x$) la corrélation de l'indice et de la variable latente (sur l'axe des $y$), presque tous les points sont au-dessus de la ligne à 45 degrés.
...
Les intervalles de confiance plus petits concernant l'indice visualisent les gains de précision résultant de l'utilisation des deux mesures.
...
De nombreuses études en sciences sociales se concentrent sur des concepts qui sont généralement supposés être latents, notamment les préférences, les connaissances et les attitudes.
...
Dans des études menées dans de multiples contextes, comme dans [l'initiative Metaketa de EGAP](http://egap.org/metaketa), les chercheurs visent à étudier la même relation causale dans plusieurs contextes nationaux.
...
Les connaissances politiques peuvent être évaluées par des questions qui demandent aux sujets de se rappeler un fait sur la politique.
...
Dans le pays $A$, cependant, cette question peut être un indicateur tout à fait approprié de la connaissance politique.
