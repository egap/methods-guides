...
==
You might expect that if $A$ causes $B$ and $B$ causes $C$ that therefore $A$ causes $C$.[^12] But there is no reason to believe that average causal relations are transitive in this way. To see why, imagine $A$ caused $B$ for men but not women and $B$ caused $C$ for women but not men. Then on average $A$ causes $B$ and $B$ causes $C$ but there may still be no one for whom $A$ causes $C$ through $B$.
