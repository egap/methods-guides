...
==
For most experimentalists, the statement "$X$ caused $Y$" means that, even though $Y$ is present, $Y$ would not have been present if $X$ were not present. This definition requires a notion of what could have happened, but did not happen. This "counterfactual" (or sometimes "difference making") approach to causality can be distinguished from the "production" approach. The "production" approach focuses on the idea of a causal connection between $A$ and $B$. Under the "production" approach, there is no notion that, just because $X$ caused $Y$, $X$ is the main reason or the only reason why $Y$ happened. 

<!-- AW: I am not sure whether the last sentence in the previous paragraph is supposed to refer to the "production" approach of causality as I have written it now or to the "counterfactual" approach. It wasn't clear in the way it was written before either and so I tried to clarify,but now I am not sure whether I clarified in the correct way. Point 3 below seems to suggest the statement would also apply  to the "counterfactual" approach but from reading briefly about the "production" approach it seems to apply there too? -->

__Technical Note:__ Statisticians employ the "potential outcomes" framework to describe counterfactual relations. In this framework, we let $Y_i(1)$ denote the outcome for unit $i$ that would be observed under one condition (e.g., if unit $i$ received a treatment) and $Y_i(0)$ the outcome that would be observed in another condition (e.g., if unit $i$ did not receive the treatment). The causal effect of the treatment for unit $i$ is then $τ_i=Y_i(1)−Y_i(0)$. A treatment has a (positive or negative) causal effect on $Y$ for unit $i$ if $Y_i(1)≠Y_i(0)$.[^2]
...
==
The "counterfactual" definition of causality requires one to be able to think through what outcomes may result in different conditions. How would things look if one party as opposed to another was elected? Everyday causal statements often fall short of this requirement in one of two ways. First, some statements do not specify clear counterfactual conditions. For example the claim that "the recession was caused by Wall Street" does not point to an obvious counterfactual--- are we to consider whether there would have been a recession if Wall Street did not exist? Or is the statement really a statement about particular actions that Wall Street could have taken but did not. If so, which actions? The validity of such statements is hard to assess, and can depend on which counterfactual conditions are implied by a statement. Perhaps a bigger problem arises when counterfactual conditions cannot even be imagined. For example, the claim that Peter got the job because he is Peter implies a consideration of what would have happened if Peter was not Peter. Alternatively, the claim that Peter got the job because he is a man requires considering Peter as a woman. The problem is that the counterfactuals in these cases imply a change not just in the condition facing an individual but in the individual themselves. To avoid such problems, some statisticians urge a restriction of causal claims to treatments that can conceivably (not necessarily practically) be manipulated.[^11] For example, while we might have difficulties with the claim that Peter got the job because he was a man, we have no such difficulties with the claim that Peter got the job because the hiring agency thought he was a man.
...
==
One strategy that people use to learn about average causal effects is to create treatment and control groups through randomization (see [10 Strategies for Figuring Out If X Caused Y](http://egap.org/resource/10-strategies-for-figuring-out-if-x-caused-y)). When doing so, researchers sometimes worry if the resulting treatment and control groups do not look the same along relevant dimensions. Experimental approaches are often justified on the grounds that random assignment helps ensure that treatment and control groups are identical "in expectation." This statement pertains to the differences between treatment and control groups that would arise on average if the procedure of randomly  assigning units to treatment and control groups was repeated many times over and over again. "In realization," however, these groups might not be identical  -- that is, in practice, the researcher randomly assigns units to treatment and control groups only once and this realized random assignment may not create identical groups. In fact, in most applications the groups will not be identical in realization.[^10]

The good news is that the argument for why differences in average outcomes across randomly assigned treatment and control groups capture average treatment effects does not rely on treatment and control groups being similar in their observed characteristics. In the absence of random assignment, treatment and control groups may look identical, but that in itself is no guarantee that comparing them will yield unbiased estimates of average treatment effects. The reason is that even groups that look identical may differ in unmeasured ways. Conversely, a randomly assigned control group might look very different from a randomly assigned treatment group. Nonetheless, the average outcome in the randomly assigned control group gives an unbiased estimate of the average potential outcome $𝔼(Y_i(0))$ that would result if all units in the experiment were assigned to control. In other words, the benefit of random assignment is that we can be certain that average outcomes in the control (treatment) group will, in expectation, be equal to average control (treated) potential outcomes across all units in the experiment.
