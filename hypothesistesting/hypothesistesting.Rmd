...
Statistical Power](https://egap.org/resource/10-things-to-know-about-statistical-power),
...
distinguish even faint signals from noise). See also [10 Things You Need to Know about Randomization Inference](https://egap.org/resource/10-things-to-know-about-randomization-inference) for more discussion of these ideas.
...
Recall from [Ten Things to Know about Causal Inference](https://egap.org/resource/10-things-to-know-about-causal-inference) that the counterfactual conceptualization of causality uses the idea of potential
...
experiment, we might [randomize the provision of information to citizens](https://egap.org/our-work-0/the-metaketa-initiative/roundtwo-taxation/), so some people will have the information and others
...
remotes::install_github("markmfredrickson/RItools@randomization-distribution")
library(RItools)
...
model after all. We do not know how reasonable the model was a priori, for example.
...
A good test rarely rejects true hypotheses (i.e. has a controlled false
positive error rate) and easily detects signal from noise (i.e. has good
statistical power, it rarely makes the error of missing the signal in the
noise).
...
but one could also assess the false positive rate for different hypotheses. We
...
this is good, it should be 5% or less. However, this is no  guarantee  of good
