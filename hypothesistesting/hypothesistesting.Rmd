...
Pour souligner la nature provisoire et théorique de ce modèle, les gens ont appelé cela une **hypothèse**, l'écrivent souvent comme "l'hypothèse nulle stricte". et utilisez le raccourci suivant : $H_0 :
y_{i,1}-y{i,0}=\tau_i$ où $\tau_i=0$ est pour toutes les unités $i$.

**Side Note:** Réfléchir à l'hypothèse nulle stricte nous fait réaliser que nous pourrions créer **d'autres modèles reliant $y_{i,1}$ et $y_{i,0}$** dans lesquels les résultats potentiels se rapportent de manière non additive ou linéaire, et où l'effet n'a pas besoin d'être nul ou identique pour toutes les unités : par exemple,
...
**L'hypothèse nulle stricte d'absence d'effet** spécifie que $\tau_i=0$ pour tous les $i$.
Et cela implique à son tour que $y_{i,0} = Y_i - Z_i \tau_i = Y_i$.
C'est-à-dire que cela implique que ce que nous observons, $Y_i$, est ce que nous observerions si chaque unité était assignée à la condition de contrôle.
Et l'implication alors, est que nous ne devrions voir aucune différence entre les groupes de traitement et de contrôle dans leurs résultats observables.

**L'hypothèse nulle faible de l'absence d'effet** spécifie que $\bar{\tau}=\bar{y}_{1} - \bar{y}_0 = 0$, et nous pouvons écrire une égalité similaire reliant la moyenne des résultats potentiels non observés à la moyennes de résultats observés dans différentes conditions de traitement.

# Les statistiques de test résument la relation entre les résultats observés et l'assignation du traitement.

Étant donné une hypothèse et un mapping des résultats non observés aux résultats observés, il faut ajouter une statistique de test au test d'hypothèse.
...
Le code ci-dessous, par exemple, montre deux statistiques de test pour une expérience avec 10 unités randomisées en deux groupes (vous pouvez appuyer sur le bouton "Code" pour voir le code R).
...
## La statistique du test des différences de rangs moyens
...
On pourrait également utiliser des versions de ces statistiques de test standardisées par leur erreur standard estimée (voir @chung2013exact pour un argument en faveur de cette statistique de test).
...
Si nous répétions l'étude et calculions la statistique du test, nous trouverions un nombre --- ce nombre refléterait le résultat de l'expérience *sous l'hypothèse*.
...
## Un exemple : tester l'hypothèse nulle stricte d'absence d'effet

Testons l'hypothèse nulle stricte d'absence d'effet.
...
Pour répéter cette opération, il suffit de permuter ou de mélanger le vecteur $Z$ donné (vous pouvez voir le code en cliquant sur le bouton "Code").
...
Ainsi, nous pouvons décrire toutes les façons dont l'expérience fonctionnerait sous cette valeur nulle en répétant simplement l'expérience (c'est-à-dire en réassignant le traitement) et en recalculant la statistique de test à chaque fois.
Le code suivant réassigne à plusieurs reprises le traitement suivant le design et calcule la statistique de test à chaque fois.
...
Et ces graphiques montrent les distributions des deux statistiques de test différentes sous l'hypothèse nulle (TODO les courbes et les graduations courtes au bas des graphiques).
Les graphiques montrent également les valeurs observées pour les statistiques de test que nous pouvons utiliser pour comparer les observations (TODO les longues lignes épaisses) à l'hypothèse (les distributions).
...
```{r fig.cap="Tester une hypothèse en utilisant un design d'expérience."}
...
text(observedMeanTZ+8,.022,"Statistique de tests observée")
...
text(observedMeanRankTZ,.45,"Statistique de tests observée")

mtext(side=3,outer=TRUE,text=expression(paste("Distributions des statistiques de test cohérentes avec le design et ", H0: y[i1]==y[i0])))
```

Pour formaliser la comparaison entre l'observation et l'hypothèse, nous pouvons calculer la proportion des expériences hypothétiques qui donnent des statistiques de test supérieures à l'expérience observée.
Dans le panneau de gauche de la figure, nous voyons qu'un large éventail des différences de moyennes entre les groupes de traitement et de contrôle sont compatibles avec un traitement sans effet (avec la plage totale entre `r min(possibleMeanDiffsH0)` et `r max(possibleMeanDiffsH0)`).
Le panneau de droite montre que remanier les résultats selon le rang avant de prendre la différence des moyennes réduit la plage des statistiques de test --- après que tous les rangs eux-mêmes passent de 1 à 10 plutôt que de 1 à 280.
...
Chaque statistique de test jette un degré de doute différent, ou quantifie le degré de surprise, pour la même hypothèse nulle d'absence d'effets.
Le résultat lui-même contient tant de bruit que la différence des moyennes des résultats selon le rang permet de mieux capter le signal que la simple différence des moyennes.
Ces données ont été générées avec des effets de traitement intégrés, donc l'hypothèse nulle d'absence d'effet est fausse, mais les informations sur les effets contiennent du bruit --- la taille de l'échantillon est petite et la distribution des résultats comporte des points aberrants et l'effet du traitement varie considérablement.

### $P$-valeurs bilatérales

Disons que nous ne savions pas à l'avance si notre expérience montrerait un effet négatif ou un effet positif.
...
Maintenant, si nous faisions cela, nous calculerions deux $p$-valeurs et, si nous le faisions de manière standard, nous prendrions le risque de nous tromper.
...
même s'il n'y a vraiment pas d'effet, 100 tests indépendants et fonctionnant bien pour l'hypothèse nulle d'absence d'effet ne donneront pas plus de 5 $p$-valeurs inférieures à 0,05.
Une solution facile pour résumer la singularité d'une expérience dans l'une ou l'autre direction est de calculer une valeur $p$ bilatérale.
...
Nous les interprétons en termes d'"extrémité" --- nous verrions seulement une différence des moyennes observée aussi loin de zéro que celle calculée à partir de nos résultats environ 18% du temps, par exemple.

**Remarque :** le test de l'hypothèse nulle stricte montré ici peut être fait sans écrire le code vous-même.
Le code que vous verrez ici (en cliquant sur le bouton "code") montre comment utiliser différents packages R pour tester des hypothèses à l'aide d'une inférence basée sur la randomisation.
...
## Un exemple: tester l'hypothèse nulle faible d'absence d'effet
...
Le code ci-dessous montre l'utilisation de la méthode des moindres carrés (MC) (`lm()` dans R) pour calculer la différence de moyennes en tant que statistique de test pour les hypothèses sur l'effet moyen.
...
C'est l'une des quelques raisons pour lesquelles on préconise, pour tester l'hypothèse nulle faible de l'absence d'effet de traitement moyen, l'utilisation des outils autres que ceux fournis par de simples méthodes des moindres carrés standardisées.
...
Ce code produit les mêmes résultats sans utiliser les moindres carrés --- après tout, nous calculons simplement les différences de moyennes et les variances de ces moyennes car elles peuvent varier au cours d'expériences répétées dans le même pool d'unités expérimentales.
...
Notez que ces tests supposent tous que la distribution de la statistique de test à travers des expériences répétées serait bien caractérisée par une distribution $t$.
La partie gauche de la figure ci-dessus montre la distribution réalisée d'une certaine manière pour que l'hypothese nulle faible soit vraie (i.e. dans ce cas l'hypothese nulle stricte est vraie aussi) :
il y a plusieurs façons pour que l'hypothese nulle faible soit vraie --- certaines sont compatibles avec des effets positifs importants sur certaines unités et des effets négatifs importants sur d'autres unités,
...
N'oubliez pas qu'une $p$-valeur utilise une statistique de test et l'idée de répétition de l'expérience pour quantifier les informations du design de recherche pour une hypothèse.
C'est le design, la statistique de test et l'hypothèse qui génèrent une distribution de probabilité.
Et ce sont les données réelles, le design et la statistique de test qui créent une valeur observée unique.
...
Disons, par exemple, que nous voyons un $p=.01$ unilatéral : ceci signifierait que dans seulement 1 expérience hypothétique sur 100 représentant l'hypothèse nulle, nous verrions un résultat aussi grand ou plus grand que notre résultat réel.
...
Ce serait bien --- après tout, une $p$-valeur à elle seule ne peut pas contrôler le comportement d'un humain adulte --- mais l'humain doit savoir que dans 1% des cas où l'hypothèse nulle est vraie, nous verrions toujours ce résultat pour ce même pool de sujets avec ce même design expérimental.
...
Dans ce cas, nous devrions également accepter de rejeter une hypothèse nulle si nous avons vu un $p \le 1/20$ ou  $p \le .05$.
...
## Que signifie rejeter une hypothèse nulle ?

Notez qu'une $p=.01$ ne reflète que la singularité des données observées par rapport à l'hypothèse --- cela signifie que la statistique de test observée semble extrême lorsqu'elle est considérée du point de vue de la distribution des statistiques de tests générées à partir de l'hypothèse nulle et du design de recherche.
Ainsi, nous pensons que $p=.01$ (comme d'autres petites $p$-valeurs ) met en doute le fait que l'hypothèse spécifique est un bon modèle des données observées.
...
de sorte qu'il est difficile de ne pas voir les résultats de ce test même si vous voulez simplement décrire les différences de moyennes dans les données mais utiliser la méthode des moindres carrés pour calculer la différence de moyennes.

## Que signifie de ne pas rejeter une hypothèse nulle ?

Notez qu'un $p=.50$ ne reflète que la singularité des données observées par rapport à l'hypothèse --- mais les données observées, dans ce cas, ne semblent pas extrêmes mais communes du point de vue de l'hypothèse nulle.
Ainsi, $p=.5$ (comme d'autres valeurs p élevées) ne nous incitent pas à douter du modèle de l'hypothèse nulle.
...
Ainsi, une seule grande valeur $p$ est un argument en faveur de l'hypothèse nulle, mais pas un argument très marqué.
...
## Comment se renseigner sur les risques d'un signal noyé dans le bruit ?

Le guide [10 choses à savoir sur la puissance statistique](https://egap.org/resource/10-things-to-know-about-statistical-power) explique comment nous voulons qu'une hypothèse rejette les faux négatifs (i.e. détecter le signal du bruit).
Lorsque nous pensons à la puissance d'un test statistique, nous devons considérer l'**hypothèse alternative**.
...
## Comment apprendre sur les faux positifs ?

Le moyen le plus simple d'en savoir plus sur les faux positifs est la simulation.
...
Comme nous pouvons le voir ici, les deux tests utilisant des permutations de traitement pour évaluer l'hypothèse nulle forte d'absence d'effet ont cette caractéristique.
Les tests pour l'hypothèse nulle faible utilisant comme statistique de test la différence des moyennes et faisant appel à la théorie des grands échantillons pour justifier l'utilisation d'une distribution $t$ n'ont pas un taux de faux positifs contrôlé : la proportion de $p$-valeurs inférieures pour une valeur donnée du seuil de rejet peut être trop élevée ou trop basse.
...
Dans ce cas particulier, pour le seuil de $\alpha=.05$, tous les tests, à l'exception du test basé sur le rang, font état de moins de 5 % de faux positifs – c'est bien, cela devrait être inférieur de 5%.
...
C'est ce qu'on appelle l'inversion du test d'hypothèse.
   (@rosenbaum2010design).
...
(@hansen:bowers:2008,@caughey2017nonparamétrique)
 ...
 - Dans les tests d'équivalence, on peut émettre l'hypothèse que deux statistiques de test sont équivalentes (c'est-à-dire que le groupe de traitement est le même que le groupe de contrôle) plutôt qu'une seule (la différence entre les deux groupes est nulle) {@hartman2018equivalence}
