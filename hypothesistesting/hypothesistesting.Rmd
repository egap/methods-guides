...
whether or not an experiment can detect a treatment effect depends not
...
When we compare what we actually observe, $t(z,Y)$, to what we
could have observed under the null (the distribution), we learn about how
extreme, weird, or surprising is our given study. And we encode this
...
null by simply repeating the experiment (i.e. re-assiging treatment) and
...
the treatment having no effects (with the range between `r min(possibleMeanDiffsH0)` and `r max(possibleMeanDiffsH0)`. Right panel shows
...
This one-sided $p$-values are `r pMeanTZ` for the simple mean difference and `r signif(pMeanRankTZ,2)` for the mean difference of the rank-transformed outcomes. Each
test statistic casts a different amount of doubt, or quantifies a different
amount of surprise, about the same null hypothesis of no effects. We would say
that it would be relatively rare to see observed values of the mean difference
-- less 9 times in 100 would we expect to see a larger difference of means if
there were no effects of the treatment on any unit in the study. The outcome
itself is so noisy that the mean
difference of the rank-transformed outcomes does a better job of picking up
the signal. These data were generated with treatment effects built in, so
...
comparisons](link) that even if there really is no effect, 100 independent and
well operating tests of that effect will yield 5 $p$-values less than .05. One
easy solution to this common situation --- of wanting to summarize extremity
of a experiment in either direction rather than just focusing on greater-than
or less-than --- is to calculate a two-sided $p$-value (Rosenbaumn 2010, Chap
2 and Cox, 1977). This, by the way, is
the standard $p$-value produced by most canned software such as `lm()` and
...
writing the code yourself. Here are a few ways.
...
## And example: Testing the weak null of no average effects
...
The left-hand panel in the figure above shows the realized distributed of one
...
In this data, however, none of those patterns will produce a reference
distribution across repeated randomizations that closely approximates a Normal
or $t$-curve if the mean difference is used as a test statistic. We will
return to this point later when we discuss the characteristics of a good test
--- one of which is a controlled false positive rate.
...
with the hypothesis --- it means that the observed test statistic looks extreme
when considered from the perspective of the distribution of tests statistics
that are generated from the null hypothesis. So, we think of $p=.01$ (and
other small p-values) as casting doubt on whether the specific hypothesis is a
good model of the observed data.
...
model after all. We don't know reasonable the model was a priori, for example.
So, a single $p$-value large $p$-value is some argument in favor of the null,
but not a very clear argument.


# Once you are using $p$-values to rejecting a hypothesis, then you will make errors
...
statistical power, has a rarely makes the error of missing the signal in the
noise).  
...
power](https://egap.org/methods-guides/10-things-you-need-know-about-statistical-power)
guide explains how we want hypotheses to rejecting false nulls (i.e. detect
...
but one could also assess the false positive rate of using different
hypotheses. We compare error rates for a few of the approaches used so far,
including the test of the weak null of no effects. The following plot shows
the proportion of $p$-values less than any given level of significance (i.e.
rejection threshold) for each of four tests. That is, this is a plot of false
positive rates for any given significance threshold. A test that has a
controlled or known false positive rate would have symbols on or below the
line. As we can see here, the two tests using permutations of treatment to
assess the sharp null of no effects (the tests plotted using the "+" and "x"
symbols, have this feature). The tests of the weak null using the mean
difference test statistic and appealing to the large sample theory to justify
the use of a $t$-distribution do not have a controlled false positive rate:
the proportion of $p$-values below any given rejection threshold can be too
high or too low.
...
except for the rank based test report about a 6% false positive rate, when it
should be 5% of less. The rank-based test statistic reports a 5% false
positive rate at $\alpha=.05%.
