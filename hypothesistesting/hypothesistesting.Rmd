...
observedMeanTZ <- meanTZ(y=Y,z=Z)
observedMeanRankTZ <- meanrankTZ(y=Y,z=Z)
```
...
```{r}
set.seed(123456)
possibleMeanDiffsH0 <- replicate(10000,meanTZ(y=Y,z=repeatExperiment(Z=Z)))
set.seed(123456)
possibleMeanRankDiffsH0 <- replicate(10000,meanrankTZ(y=Y,z=repeatExperiment(Z=Z)))
```
...
par(mfrow=c(1,2),mgp=c(1.5,.5,0),mar=c(2,3,0,0),oma=c(0,0,3,0))
...
the treatment having no effects (with the range between `r
min(possibleMeanDiffsH0)` and `r max(possibleMeanDiffsH0)`. Right panel shows
...
This one-sided $p$-values are `r pMeanTZ` for the simple mean difference and `r
pMeanRankTZ` for the mean difference of the rank-transformed outcomes. Each
...
In this case the two-sided $p$-values are `r p2SidedMeanTZ` and `r
p2SidedMeanRankTZ` for the simple mean differences and means differences of
ranks respectively. We interpret them in terms of "extremity" --- we would
only see an observed mean difference as far away from zero as the one manifest
in our results roughly 18% of the time, for example.
