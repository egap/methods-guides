...
In this guide, the only type of estimand I consider is the overall average treatment effect (ATE).
...
Finally, the researcher must also choose a standard error estimator if she wants
to summarize how the estimates might
vary if the research design or  underlying data generating process were repeated.

First, to provide context, let's consider an example.
The researcher decides their *estimand* will be the average treatment effect for the pool of subjects in the experiment: here, the researchers do not observe how the control outcomes of the subjects assigned to treatment or the treated outcomes of the subjects assigned to control. The estimand is a counterfactual quantity in this case. The researchers observe all of the subjects for whom they want to estimate an effect. This kind of estimand might be called a finite-population estimand. See [10 Things to Know About Causal Inference](https://egap.org/resource/10-things-to-know-about-causal-inference/){target="_blank"} and [10 Types of Treatment Effect You Should Know About](https://egap.org/resource/10-types-of-treatment-effect-you-should-know-about/){target="_blank"} for a discussion of other common estimands.
...
I follow the Neyman-Rubin potential outcomes notation in this guide (@Neyman1923, @Imbens2015).
...
I define $B_{ij}$ as the unit-level intention-to-treat effect (ignoring any issues of noncompliance to the ITT is the ATE as we have been discussing previously), $B_{ij} =  Y_{ij}(1) - Y_{ij}(0)$.
...
For example, in a clinical trial, a researcher may decide to block based on gender or specific age categories in order to increase statistical power overall or ensure statistical power to assess effects within subgroups (such as those defined by time of entering the study, or defined by other important covariates that might predict the outcome) [@moore2012multivariate;@moore2013;@bowers2011mem].
...
For example, in a study of a new online math tool for high school students,
randomization occurs at the student level within blocks defined by
sites/schools. For example, perhaps half of students at each school are assigned
to the status quo (no additional math practice), and half are assigned to the
active treatment (an offer of additional math practice at home using an online
tool).
...
treatment. [This video explains the difference between cluster and
block-randomized designs](https://youtu.be/bL2U9z8hX1k){target="_blank"}. In a multisite trial
trial treatment is assigned **within a block to individual units**. In a
cluster-randomized trial treatment is assigned to groups of
units. Some designs [combine cluster- and
block-randomization](https://declaredesign.org/r/designlibrary/reference/block_cluster_two_arm_designer.html){target="_blank"}
.

Another design that is not a multisite or block-randomized trial is  an
experiment that takes place in only one school and assigns individual students
to treatment and control: this study has only one site and thus differences
...
Randomizing with purposefully created blocks or pre-existing sites also helps
analysts learn about how the effects of the treatment may vary across the sites
or groups of people categorized into the blocks. If a new treatment should help
the lowest performing students most, but in any given study most students are
not the lowest performing, then researchers may prefer to create blocks of
students within schools with the students divided by their previous performance.
This blocking within site would allow comparisons of the treatment effects on
the relatively rare lowest performing students with the treatment effects on the
relatively rare highest performing students.

## Why not block?

Often, in a multisite trial with treatment administered by site administrators
(like principals of schools), an analyst has no choice but to randomize within
site. In other studies, the construction and choice of blocking criteria is a
choice. @Pashley2022 shows that blocking is generally beneficial, but also
explores settings in which it may be harmful. Blocking does result in fewer
degrees of freedom, but in practice this reduction is rarely an issue, unless an
experiment is very small [@Imai2008]. Any use of blocking requires that an
analyst keep track of the blocks and also that an analyst reflect the blocks in
subsequent analysis: in many circumstances estimating average treatment effects
from a block-randomized experiment while ignoring the blocks will yield biased
estimates of the underlying targeted estimands.

<!-- TODO: Not sure that this discussion belongs here.
...
The ICC is defined as the ratio of the variance at the site level divided by the overall variance of the individual outcomes. Although this quantity is also used in the design and analysis of cluster-randomized trials, it plays a different role in block-randomized trials depending on the target of inference chosen by the analyst.
-->
...
The researcher may want to focus on the **finite population** --- only those units in the experimental pool or sample --- or expand their estimand to consider the **super population** --- units not included in the experiment.
...
Similarly, if an organization is evaluating itself and includes all of its own sites, they would use a finite population framework. Many field experiments use a finite population framework because the units and sites available for study may not arrive via any known or replicable sampling process: they might be called a "convenience sample". An additional common case of a finite population framework is for proof-of-concept or pilot studies.
...
I will discuss this in more detail later in the guide, but one of the main consequences of the choice of population framework is the amount of uncertainty in the final estimates.
When accounting for sites randomly sampled via a known sampling process from a super population, we naturally have an additional source of uncertainty deriving from which units were selected for the study at hand: randomization to treatment is one source, and sampling from the population is another source.
...
Multilevel modeling is an explicitly model-based approach to statistical
inference --- where the researcher estimates the parameter in a likelihood
function that is chosen to represent the natural stochastic process that
generates the outcomes in the study. Linear regression can be used to simply
calculate difference of means (as a tool of design-based statistical inference)
or as the estimator of the paramters of a  Normal or Gaussian data generating
process (as a tool of model-based statistical inference).See Don Rubin's paper
on "Formal modes of statistical inference for causal effects"[@rubin:1990b] for more discussion of the differences between design- and model-based approaches to statistical inference.
...
When assuming *random effects*, as in a multilevel model, each site's parameter is assumed to be drawn from a shared distribution of site impacts --- where software nearly always uses a Normal distribution to model the site-specific impacts.
...
In the super population framework, estimators look at the variation *between* sites to "capture both any within-site estimation error along with the uncertainty associated with sampling sites from a larger population." <!-- TODO: citation here? -->
...
Huber-White standard errors correspond to the finite population framework, while cluster robust standard errors correspond to the super population framework. <!-- TODO: Is this always right? Isn't the CR2 Cluster Robust SE approximating the randomization distribution of cluster-level assignment? If so, then are RCSEs sometimes focusing on finite-pop? If so, then what RCSEs focus on the super-pop and why? -->
...
Huber-White standard errors allow for heteroscedasticity in the residuals while still assuming that sites are fixed, which fits into a finite population framework: in fact @lin2013agnostic shows that the standard error derived by @Neyman1923 on finite-population and design-based principles is the same as the HC2 standard error.
In contrast, cluster robust standard errors allow for heteroscedasticity by assuming that sites themselves are sampled $iid$ from a super population. <!-- TODO: Confirm this or complicate it ok? -->
...
Most likely, "this implies that the potential bias in the bias-precision tradeoff to the fixed effect estimators is negligible in practice." Other [authors](https://egap.org/resource/sd-block-rand/){target="_blank"} have been able to create situations in which the bias-precision tradeoff is more severe.
