...
Resumen==
Esta guía introduce la idea de un plan de análisis previo (PAP), ofrece un modelo y preguntas orientativas para redactar planes de preanálisis en sus estudios, y explica cuáles son los usos de un plan de análisis previo. Al final de este documento se ofrecen enlaces a registros de PAP con ejemplos de planes.
...
[^TOP]: Los PAP se promueven como parte de las [Pautas para la promoción de transparencia y apertura (TOP)] (https://osf.io/4kdbm/?_ga=2.259736077.62256863.1547257566-317545181.1501862097) [@nosek2015], publicadas en [Science](http://science.sciencemag.org/content/348/6242/1422.full), donde las principales revistas de ciencias sociales se comprometen a aplicar las directrices TOP. 

Los objetivos del PAP son: mejorar las opciones de diseño de la investigación, aumentar la transparencia de la investigación y permitir que otros académicos repliquen su análisis.  En consecuencia, recomendamos centrar el PAP en los detalles analíticos que le ayuden a analizar su estudio y permitan a otros investigadores replicar su análisis. Puede incluirse una breve sección sobre teoría, siempre y cuando ayude a articular las hipótesis, pero no es necesario incluir un resumen detallado de teoría y literatura. No es necesario que el PAP incluya la primera parte de un artículo académico, si estas secciones no sirven para reflexionar sobre su análisis, o si tampoco ayuda a que los lectores repliquen su análisis.

En las siguientes secciones proporcionaremos guías sobre los detalles que se deben incluir en los PAP, incluyendo textos de ejemplo. También recomendamos que incluya la mayor cantidad posible de código y análisis de datos simulados. [^DD] Muchos PAPs no podrán incluir todo lo que aparece en nuestra lista, pero un PAP debería incluir, como mínimo, la lista completa de hipótesis que se pretenden probar, la forma en que se medirán las variables relevantes para dichas hipótesis y un sello de tiempo verificable.

[^DD]: Recursos como el proyecto DeclareDesign pueden ayudarle a simular y analizar datos falsos que imitan los datos reales que su proyecto recogerá. El [análisis de poder](http://egap.org/resource/10-things-to-know-about-statistical-power), un componente importante de un PAP, requiere datos simulados.


2. ¿Qué hay en un PAP? Resumen del diseño de estudio.
==

La primera sección de un PAP debe presentar un breve resumen del diseño de su estudio.  Si el estudio es un experimento, describa el procedimiento de aleatorización y la intervención o procedimiento experimental. Si el estudio no es un experimento, describa los datos. Estas descripciones deben incluir: (1) unidad de análisis, población y criterios de inclusión/exclusión, (2) tipo de método (observacional, experimental, cuasi-experimental), (3) intervención experimental o variable explicativa y (4) variables de resultado de interés.

<p class="comment">
**Ejemplo:** 
Utilicemos una versión simplificada de una intervención en Malawi como ejemplo práctico. <br><br>
Utilizamos un experimento de aleatorización en bloque para evaluar el efecto de la _información y la prestación de servicios públicos_ (variable explicativa) sobre el _cumplimiento fiscal_ (variable de resultado) en Malawi, donde la prestación de servicios es baja y el incumplimiento en el pago de impuestos es alto.<br><br>
Nuestra unidad de análisis es el hogar habitado por su propietario en una ciudad de Malawi.  Excluimos a los inquilinos porque sólo los propietarios pagan los impuestos municipales pertinentes para nuestro estudio.  Agrupamos los hogares en 80 unidades a nivel de barrio, de aproximadamente 20 hogares cada una.  Cada vecindario es un bloque, de modo que 10 hogares de cada vecindario se asignan al tratamiento y 10 hogares al control.<br><br>
Nuestra intervención es el suministro de información y servicios públicos. Llevamos a cabo esta intervención al proporcionar dos recogidas de basura gratuitas, la visita de un encuestador para hablar de cómo el pago de impuestos financia los servicios de la ciudad, como por ejemplo dicha recogida de residuos, y un folleto con más información sobre el pago de impuestos. <br><br>
Nuestra principal variable de resultado es el cumplimiento fiscal, que estudiamos como pagos de impuestos municipales. Realizamos una encuesta y recogemos datos administrativos sobre el pago de impuestos tanto antes como después de la intervención.
</p>


3. ¿Qué hay en un PAP? Hipótesis.
==

El PAP debe especificar sus hipótesis: cuáles son la(s) relación(es) que espera observar entre variables. La formulación de una hipótesis debe dejar claro si se trabaja con pruebas de una o dos colas (por ejemplo, predecir un aumento, una disminución o un cambio en la variable de resultado).

Hay dos tipos de hipótesis que debe considerar en su PAP: confirmatorias y exploratorias. **Las hipótesis confirmatorias** son el objetivo principal de la mayoría de los estudios; son las hipótesis que su estudio está diseñado para comprobar. Sus análisis sobre estas hipótesis tendrán un gran poder estadístico y, por lo general, tendrán una teoría sólida que conducirá a estas hipótesis _a priori_.
...
Algunas personas prefieren enumerar pocas hipótesis y otras prefieren enumerar muchas.  Como norma, debe intentar incluir cuantas hipótesis pueda que se relacionen con su teoría o intervención. [^FisherQuote] Esta puede tratarse de una sola variable de resultado, pero si su intervención experimental o teoría hace predicciones sobre 8 variables de resultado, debe enumerar las hipótesis para esas 8 variables de resultado. Si su intervención o teoría experimental presenta mecanismos específicos a través de los cuales la variable explicativa afecta a una o varias variables de resultado, esos mecanismos deben quedar claros en la hipótesis.
...
Además de una mejora general en la actitud hacia el gobierno, esperamos que los residentes puedan modificar sus actitudes en otras cuestiones específicas. Nos gustaría buscar cuáles asuntos pueden estar generando cambios de actitud con las siguientes hipótesis exploratorias:
<ul>
...
4. ¿Qué hay en un PAP? Construcción de índices y mediciones.
== 

El PAP debe especificar la forma en que se mide u operacionalizan las variables de interés, incluidos los resultados, las covariables y las variables explicativas. Estas operacionalizaciones pueden ser incluidas en una sección propia, o la operacionalización de las variables puede incluirse según la hipótesis para la que cada variable es relevante.

El PAP debe enumerar la forma en que se mide cada variable (como preguntas de encuestas o entrevistas, datos administrativos, mediciones de comportamiento/observación, etc.) y con qué hipótesis se relaciona la variable. Los detalles de estas mediciones, como la redacción precisa de las preguntas de la encuesta, deben incluirse en esta sección o en un apéndice. Si se utilizan índices o factores, o se combinan variables de resultado entre sí, debe especificar cómo se construirán las variables de resultado combinadas. Si de alguna manera se manipulan o transforman las variables de resultado (como el registro de una variable), describa el proceso de manipulación o transformación.

Recomendamos que se incluya código en su PAP, para demostrar cómo planea ejecutar toda la transformación de datos.
...
También creamos un índice de "actitudes hacia el gobierno" utilizando ponderaciones de covarianza inversa (Inverse Covariance Weights, ICW) de 6 preguntas de la encuesta, en donde los valores más altos significan actitudes más positivas hacia el gobierno. El índice ICW pondera las preguntas de referencia inicial por la covarianza de las respuestas en el grupo de control de referencia inicial, y pondera las preguntas de la línea final por la covarianza de las respuestas en el grupo de control en la línea final. A continuación estandarizamos la escala de ICW por desviación estándar, de modo que un 1 en la referencia inicial significa 1 desviación estándar (DE) por encima de la media en la referencia inicial, y un 1 en la línea final significa 1 DE por encima de la media en la línea final.
</p>


5. ¿Qué hay en un PAP? Procedimiento de estimación.
==

Ya que ha descrito el diseño del estudio, las hipótesis y las variables, está listo para discutir los procedimientos de pruebas y estimaciones.  

Esta sección debe especificar de manera clara lo que se está estimando (es decir, el estimando) y cómo se pretende estimarlo (es decir, el estimador). Por ejemplo, muchos estudios estiman el efecto promedio del tratamiento de una intervención experimental utilizando la regresión lineal MCO (mínimos cuadrados ordinarios)  como estimador.^[También podría estar interesado en estimar otro tipo de efectos.  Consulte esta guía sobre tipos de efectos de tratamiento (https://egap.org/es/methods-guides/10-types-treatment-effect-you-should-know-about) para obtener más información sobre los tipos de efectos].  Establezca claramente la especificación de su modelo, incluyendo sus variables de resultado, las variables explicativas y las covariables, así como el estadístico de prueba.
...
Estimamos el efecto de la campaña de información y del servicio de recogida de basuras sobre el pago de los impuestos municipales entre los residentes con un análisis de la r intención de tratar.. Nuestro estimando es el efecto promedio del tratamiento.br><br>
Si tenemos un equilibrio entre las variables de resultado de referencia inicial y final, podremos usar el siguiente estimador para estimar el efecto promedio de tratamiento:<br><br>
$Y_{i,j} = \beta_0 + \beta_1Z_{i,j} + X_{i,j}+ \epsilon_{i,j}$<br><br>
donde $i$ es el individuo en el vecindario $j$, $Z$ es el indicador del tratamiento, y $Y$ la variable de resultado. $X$ es la variable de resultado de referencia inicial para el individuo $i$. Usaremos ponderaciones de regression proporsionales al tamaño de los vecindarios $j$. Si las variables de resultado de referencia inicial y final no están equilibradas, usaremos la puntuación de cambio, $Y_i = Y_{i,endline} - Y_{i,baseline}$ y no usaremos $X$. 
</p>

6. ¿Qué hay en un PAP? Criterios de Inferencia{#infcrit}
==

Los criterios de inferencia son reglas de decisión que determinan la detectabilidad de los efectos (es decir, si una variable explicativa afecta realmente a una variable de resultado). Se requiere tomar varias decisiones para establecer los criterios de inferencia, respecto a cuándo creer que el efecto estimado es "estadísticamente y significativamente diferente" con respecto a las hipótesis nulas. Se recomienda especificar y justificar de manera clara las respuestas a las siguientes preguntas:

1. ¿Qué tipo de errores estándar va a utilizar? ¿Por qué utilizará este tipo de errores estándar? ^[Por ejemplo, se podrían calcular los errores estándar y los valores $p$ utilizando la [inferencia de aleatorización] basada en permutaciones (https://egap.org/resource/10-things-to-know-about-randomization-inference). O se podrían aproximar los errores estándar y los valores $p$ utilizando métodos analíticos [@samii2012]].
...
Puede optar por utilizar varios procedimientos para los criterios de inferencia. Por ejemplo, puede querer utilizar tanto los ajustes FWER como FDR para las comparaciones múltiples, y comparar entre ambos. O podría querer utilizar los valores $p$ de la inferencia de aleatorización como una comprobación de los valores $p$ de una distribución nula que se supone como normal. Si decide utilizar varios procedimientos, debe especificar cómo interpretará estos resultados, si los distintos procedimientos llegan a conclusiones diferentes.

<p class="comment">
**Ejemplo:**<br><br>
Utilizamos los errores estándar HC2 con nuestro experimento aleatorio en bloque, porque es equivalente al estimador de varianza de Neyman basado en la aleatorización [@samii2012]. Esperamos que el grupo de tratamiento pague más impuestos municipales que el grupo de control, por lo tanto utilizamos una prueba de una cola en la que $H_1 > H_0$. Establecemos $\alpha = 0,05$ y rechazamos la nulidad cuando el valor $p$ es inferior a 0,05. Como sólo tenemos una hipótesis confirmatoria, no ajustamos para comparaciones múltiples.
<br><br>
Como comprobación de los errores estándar de HC2, también calculamos los valores $p$ utilizando directamente la [inferencia de aleatorización](https://egap.org/resource/10-things-to-know-about-randomization-inference), con la diferencia en medios como prueba estadística.
</p>

7. ¿Qué hay en un PAP? Procedimientos para Problemas de Datos.
...
Dos problemas de datos comunes son (1) puntos de datos extremos y (2) datos faltantes. **Los puntos extremos** pueden representar un verdadero valor atípico (una unidad con variables de resultado mucho mayores o menores que los de otras unidades) o pueden deberse también a errores en la recogida de datos. Las tabletas de encuesta, los equipos de grabación, los raspadores de páginas web y otras herramientas de recopilación de datos pueden registrar puntos extremos debido a fallos técnicos. Puede ser difícil saber si los puntos extremos representan datos recogidos veraces, o si se deben a errores en la introducción de datos, pero es importante especificar en el PAP cuándo se espera ver errores en la recopilación de datos y cuáles serán los procedimientos para trabajar con puntos extremos.

**Los datos faltantes** pueden presentarse de dos formas: covariables faltantes y variables de resultados faltantes. También es importante especificar en el PAP cuándo se espera ver variables de resultado o covariables faltantes y cuales serán los procedimientos para tratarlos. Los puntos extremos y los puntos faltantes que son aleatorios serán menos problemáticos que los puntos faltantes o extremos que parecen tener un patrón.
...
- ¿Qué dificultades pueden causar estos puntos de datos extremos/covariables ausentes/variables de resultado faltantes? ¿Cómo puede adelantarse para mitigar estos problemas de datos?
  ...
  - ¿Qué procedimientos utilizaría para tratar los puntos extremos/falta de datos y cómo justificaría el uso de esos procedimientos?  
      - Si los datos extremos o faltantes no son aleatorios, recomendamos incluir algún tipo de análisis de límites para determinar los límites de su estimación con puntos extremos o faltantes. Por ejemplo, los límites de valor extremo pueden ayudarle a determinar el rango de un efecto promedio del tratamiento, estableciendo todas las variables de resultado ausentes en el grupo de tratamiento con la mayor variable de resultado posible, y estableciendo todas las variables de resultados ausentes en el mismo grupo de control con el menor resultado posible [@gerber2012field].
      - Si los datos extremos o faltantes son aleatorios, recomendamos imputar los puntos extremos o los datos faltantes. También se pueden descartar estos datos si los datos extremos o faltantes son aleatorios, y se debería especificar las condiciones bajo las cuales se decide descartar dichos datos. Por ejemplo, considere la posibilidad de establecer un umbral para una covariable faltante, de modo que si el porcentaje de datos faltantes de la covariable excede el umbral, se pueda eliminar dicha covariable.
...
Si al realizar nuestra encuesta los encuestados no responden a una o más preguntas que miden una variable de resultado, nuestro procedimiento es el siguiente:
<ol>
...
<li>If $p < .05$ para la evaluación de la relación entre el tratamiento y las variables de resultado faltantes, presentaremos un análisis de límites de valores extremos, en el que fijamos todas las variables de resultado faltantes para el tratamiento en el máximo del bloque, y todas las variables de resultado faltantes para el control en el mínimo del bloque.</li>
<li>If $p \geq 0.5$ para la evaluación de la relación entre el tratamiento y las variables de resultado faltantes, imputaremos las variables de resultado faltantes utilizando la media de la subcategoría de asignación por bloques.</li>
</ol>
Si aún tenemos preguntas sobre la variable de resultado, con tasas particularmente altas de no-respuesta del 10 por ciento, entonces debemos:
<ol>
...
<li>Considerar el descartar estas variables de resultado y catalogar las razones de esta decisión.</li>
</ol>
</ol>

8. ¿Qué hay en un PAP? Análisis de poder.
==

En los PAP se suele incluir un [análisis de poder] (https://egap.org/resource/10-things-to-know-about-statistical-power).  El poder estadístico es la probabilidad de que su estudio pueda detectar un efecto, si es que hay un efecto que detectar.
...
Falta de transparencia en las investigaciones ha provocado varios problemas, entre ellos la crisis de replicación.
...
- [Registro AEA](https://www.socialscienceregistry.org) (for RCTs only)
