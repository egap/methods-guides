...
Résumé
==
Ce guide présente l'idée du plan de pré-analyse (PAP), propose un modèle et des questions d'orientation pour rédiger des plans de pré-analyse pour vos études et explique les utilisations d'un plan de pré-analyse.
...
De nombreux PAP ne pourront pas tout inclure sur notre liste, mais un PAP devrait, au minimum, inclure la liste complète des hypothèses que vous avez l'intention de tester, comment vous allez mesurer les variables pertinentes pour ces hypothèses et un horodatage vérifiable.
...
Si les résultats de référence et de fin ne sont pas équilibrés, nous utiliserons le score de changement, $Y_i = Y_{i,fin} - Y_{i,référence}$ et nous n'utiliserons pas $X$.
</p>
...
Par exemple, vous pouvez utiliser les ajustements de taux d'erreurs par famille (family-wise error rate, FWER) et de taux de fausse découverte (false discovery rate, FDR) pour plusieurs comparaisons et comparer les deux.
...
Les procédures courantes pour traiter les données manquantes ou les points extrêmes sont 1) l'analyse des bornes ; 2) l'imputation; 3) l'abandon des observations.
...
Par exemple, les bornes des valeurs extrêmes peuvent vous aider à déterminer la plage de l'effet moyen du traitement en attribuant pour tous les résultats manquants dans le groupe de traitement le plus grand résultat possible et en attribuant pour tous les résultats manquants dans le groupe de contrôle le plus petit résultat possible [@gerber2012field].
...
<li>Envisager d'abandonner ces résultats d'enquête et répertorier la raison de votre décision.</li>
</ol>
</ol>
...
En fait, vous pouvez combiner le code pour une analyse de puissance avec le code écrit pour d'autres sections du PAP et créer un PAP de calcul.
...
La création d'un PAP oblige le chercheur à élaborer les nombreuses décisions de design qui devront être prises lors de la réalisation d'une étude.
...
Certaines personnes soutiennent qu'un PAP verrouille l'étude dans une stratégie de design, d'intervention et d'estimation particulière alors que les détails de la stratégie de design, d'intervention et d'estimation peuvent changer pendant la réalisation d'une étude.
