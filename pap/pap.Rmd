...
<!-- author: "Auteur du guide des méthodess: Nuole (Lula) Chen et Christopher Grady" -->

Résumé
==
Ce guide présente l'idée d'un plan de pré-analyse (PAP), propose un modèle et des questions d'orientation pour rédiger des plans de pré-analyse pour vos études et explique les utilisations d'un plan de pré-analyse.
Des liens vers les registres PAP avec des exemples de plans sont fournis à la fin de ce document.

1 Qu'est-ce qu'un plan de pré-analyse (PAP)
==

Un PAP est un document qui formalise et déclare le design et le plan d'analyse de votre étude.
Il est rédigé avant la réalisation de l'analyse et est généralement enregistré sur un site Web tiers.[^TOP]

[^TOP] : les PAP sont encouragées dans le cadre des [directives pour la promotion de la transparence et de l'ouverture (Transparency and Openness Promotion, TOP)](https://osf.io/4kdbm/?_ga=2.259736077.62256863.1547257566-317545181.1501862097) [@nosek2015], publié dans [Science](http://science.sciencemag.org/content/348/6242/1422.full), avec les principales revues scientifiques pour les sciences sociales s'engageant à mettre en œuvre les lignes directrices TOP.

Les objectifs du PAP sont d'améliorer les choix de design de recherche, d'accroître la transparence de la recherche et de permettre à d'autres chercheurs de reproduire votre analyse.
...
De nombreux PAP ne pourront pas tout inclure sur notre liste, mais un PAP devrait, au minimum, inclure la liste complète des hypothèses qui vous avez l'intention de tester, comment vous allez mesurer les variables pertinentes pour ces hypothèses et un horodatage vérifiable.

[^DD]: Des ressources telles que le projet DeclareDesign peuvent vous aider à simuler et à analyser de fausses données qui imitent les données réelles que votre projet rassemblera.
...
2 Qu'y a-t-il dans un PAP? Aperçu du design de l'étude
==

La première section d'un PAP doit fournir un bref aperçu du design de votre étude.
...
<p class="comment">
**Exemple:**
Prenons une version simplifiée d'une intervention au Malawi comme exemple courant. <br><br>
Nous utilisons une expérience randomisée par bloc pour évaluer l'effet de _la fourniture d'informations et de services publics_ (variable explicative) sur la _conformité fiscale_ (résultat) au Malawi, où la prestation de services est faible et la non-conformité fiscale élevée.<br><br>
Notre unité d'analyse est le ménage occupé par les propriétaires dans une ville du Malawi.
Nous excluons les locataires car seuls les propriétaires paient les taxes de séjour pertinentes pour notre étude.
Nous mettons les ménages dans 80 groupes au niveau du quartier d'environ 20 ménages chacun.
...
Notre intervention est la fourniture d'informations et de services publics.
Nous mettons en œuvre cette intervention en offrant deux collectes de déchets gratuites, une visite d'un solliciteur pour discuter de la façon dont les paiements d'impôts financent les services municipaux comme la collecte des déchets, et une brochure avec plus d'informations sur les paiements d'impôts.<br><br>
...
**Les hypothèses de confirmation** sont au cœur de la plupart des études ; ce sont les hypothèses que votre étude est conçue pour tester.
...
<li>H : Le traitement augmentera les croyances dans la capacité de service de la ville parmi les ménages de traitement par rapport aux ménages de contrôle.</li>
...
Notre variable explicative est l'assignation du traitement, où les individus assignés au traitement reçoivent $1$ et les individus affectés au contrôle reçoivent $0$.<br><br>
Nous créons également un indice de "l'attitude envers le gouvernement" en utilisant des poids de covariance inverse (ICW) de 6 questions d'enquête où des valeurs plus élevées signifient des attitudes plus positives envers le gouvernement.
L'indice ICW pondère les questions de base par la covariance des réponses dans le groupe de contrôle et pondère les questions post-traitement par la covariance des réponses dans le groupe de contrôle post-traitement.
Nous standardisons ensuite l'échelle ICW par l'écart type de sorte qu'on peut comparer les réponses pré- et post-traitement.
</p>

5 Qu'y a-t-il dans un PAP? Procédure d'estimation
==

Maintenant que vous avez décrit votre design d'étude, vos hypothèses et vos variables, vous êtes prêt à discuter de vos procédures de test et d'estimation.

Cette section doit clairement spécifier ce que vous estimez (c'est-à-dire l'estimande) et comment vous avez l'intention de l'estimer (c'est-à-dire l'estimateur).
Par exemple, de nombreuses études estiment l'effet moyen du traitement d'une intervention expérimentale en utilisant la régression linéaire MCO comme estimateur.
...
Décrivez clairement la spécification de votre modèle, y compris vos résultats, les variables explicatives et les covariables, ainsi que vos statistiques de test.
...
Nous estimons l'effet de la campagne d'information et du service de collecte des déchets sur le paiement des taxes d'habitation chez les résidents avec une analyse d'effet d'intention de traiter.
...
Si les résultats de référence et de fin ne sont pas équilibrés, nous utiliserons le score de changement, $Y_i = Y_{i,endline} - Y_{i,baseline}$ et nous n'utiliserons pas $X$.
</p>
...
1. Quel type d'erreurs standard utiliserez-vous ? Pourquoi utilisez-vous ce type d'erreurs standard ?^[Par exemple, vous pouvez calculer les erreurs standard et les $p$-valeurs en utilisant [l'inférence de randomisation](https://egap.org/resource/10-things-to-know-about-randomization-inference) basée sur la permutation.
Ou vous pouvez approcher précisement les erreurs standard et les $p$-valeurs en utilisant des méthodes analytiques [@samii2012]]
...
Par exemple, vous pouvez utiliser les ajustements FWER et FDR pour plusieurs comparaisons et comparer les deux.
...
<p class="comment">
**Exemple :**<br><br>
Nous utilisons les erreurs standard HC2 avec notre expérience randomisée par bloc car elle équivaut à l'estimateur de variance Neyman basé sur la randomisation [@samii2012].
...
Comme nous n'avons qu'une seule hypothèse de confirmation, nous n'ajustons pas de comparaisons multiples.
<br><br>
Pour vérifier les erreurs standard HC2, nous calculons également les $p$-valeurs directement en utilisant [l'inférence de randomisation](https://egap.org/resource/10-things-to-know-about-randomization-inference), avec la différence des moyennes comme statistique de test.
</p>
...
**Les points extrêmes** peuvent représenter une vraie valeur aberrante - une unité avec des résultats beaucoup plus grands ou plus petits que d'autres unités - ou peuvent se produire en raison d'erreurs de collecte de données.
Les tablettes de sondage, les outils d'enregistrement, les web scrapers et d'autres outils de collecte de données peuvent enregistrer des points extrêmes en raison de problèmes techniques.
Il peut être difficile de savoir si les points extrêmes représentent de vraies données collectées ou s'ils sont dus à des erreurs de saisie de données, mais il est important de spécifier dans le PAP quand vous vous attendez à voir des erreurs de collecte de données et les procédures pour traiter les points extrêmes.

**Les données manquantes** peuvent se présenter sous deux formes : covariables manquantes et/ou résultats manquants.
Il est également important de spécifier quand vous vous attendez à voir des résultats ou une covariable manquants et les procédures pour les traiter dans votre PAP.

Les points extrêmes/manquants qui sont aléatoires seront moins problématiques que les points extrêmes/manquants qui semblent avoir un motif.

Les procédures courantes pour traiter les données manquantes ou les points extrêmes sont 1) l'analyse des bornes ; 2) imputation; 3) abandon des observations.
...
- Quels problèmes peuvent être à l'origine de ces points de données extrêmes/covariables manquantes/résultats manquants ?
  Que pouvez-vous faire à l'avance pour atténuer ces problèmes de données ?
   - Comment jugeriez-vous si les données extrêmes/manquantes sont vraisemblablement aléatoires ?
     (i.e. les données extrêmes/les données manquantes sont-elles corrélées au traitement, à des covariables/sous-groupes spécifiques ou aux résultats ?
   - Quelles procédures utiliserez-vous pour traiter les données extrêmes/manquants et comment justifiez-vous l'utilisation de votre procédure ?

       - Si les données extrêmes/manquantes ne sont pas aléatoires, nous vous recommandons d'inclure une sorte d'analyse des bornes pour déterminer les bornes de votre estimation avec des données extrêmes/manquantes.
Par exemple, les bornes des valeurs extrêmes peuvent vous aider à déterminer la plage de leffet moyen du traitement en attribuant pour tous les résultats manquants dans le groupe de traitement le plus grand résultat possible et en attribuant pour tous les résultats manquants dans le groupe de contrôle le plus petit résultat possible [@gerber2012field].

      - Si les données extrêmes/manquantes sont aléatoires, nous recommandons d'imputer les points extrêmes ou les données manquantes.
...
<li>Si $p < 0,05$ pour l'évaluation de la relation entre le traitement et les résultats manquants, nous ajouterons une analyse des limites des valeurs extrêmes dans laquelle nous fixons tous les résultats manquants pour le traitement au maximum du bloc et tous les résultats manquants pour contrôle au minimum du bloc.</li>
...
<li>Décrire la relation entre la non-réponse à cette question et d'autres données sur les personnes via </li>
  ...
  <li>une sélection de variables à l'aide d'un modèle de réseau élastique adaptatif avec des paramètres de réglage choisis par une validation croisée répétée 10 fois au sein du sous-groupe du design.</li>
  </ul>
<li>Envisagez d'abandonner ces résultats d'enquête et répertoriez la raison de votre décision.</li>
</ol>
</ol>
...
À bien des égards, l'analyse de la puissance de calcul implémente toutes les spécifications que vous avez faites dans votre PAP dans le code.
En fait, vous pouvez combiner le code pour une analyse de puissance de calcul avec le code écrit pour d'autres sections du PAP et créer un PAP de calcul.
Un tel outil, qui peut être implémenté avec un logiciel comme [DeclareDesign](https://declaredesign.org), peut alors vous aider à diagnostiquer des problèmes potentiels, vous en apprendre davantage sur votre design et renforcer votre PAP.
...
*1. Les PAP aident à contrer la crise de réplication.*

Un manque de transparence de la recherche a conduit à plusieurs problèmes, dont l'un est la crise de réplication.

La crise de réplication est que de nombreuses études universitaires sont difficiles ou impossibles à reproduire.
Les PAP réduisent le nombre d'études qui ne sont pas reproductibles en raison de mauvaises pratiques en matière de données, telles que le data mining jusqu'à trouver des données montrant une relation causale fallacieuse ou le trucage involontaire des $p$-valeurs.
Les PAP augmentent également le nombre d'études qui élaborent les procédures nécessaires à la réplication, ce qui permet de tenter de reproduire ces études.
...
Ces registres permettent aux universitaires et aux praticiens d'identifier si les effets publiés sur un sujet représentent avec précision les effets trouvés dans les études non publiées, ou si les effets publiés diffèrent des études non publiées.
...
Le pré-enregistrement des études de confirmation ou d'exploration permet en outre aux chercheurs de savoir si les recherches futures devraient s'appuyer sur l'étude ou si les recherches futures devraient corroborer l'étude.
...
La création d'un PAP oblige le chercheur à élaborer les nombreuses décisions de design qui doivent être prises lors de la réalisation d'une étude.
Dans le cas des études d'observation, la modification ultérieure de ces décisions de design n'est pas un problème.
Mais pour les études expérimentales ou d'autres études utilisant la collecte de données originales, le chercheur a la chance de collecter les données nécessaires à l'analyse.
Les PAP aident à garantir que le chercheur réfléchit à toutes les décisions et collecte les bonnes données.
...
Certaines personnes soutiennent qu'un PAP verrouille l'étude dans une stratégie de design, d'intervention et d'estimation particulière même si les détails de la stratégie de design, d'intervention et d'estimation peuvent changer pendant la réalisation d'une étude.
...
Les chercheurs doivent se rappeler que *tout plan de pré-analyse peut être révisé* ! Votre premier PAP ne vous enferme pas dans un design de recherche, une variable de résultat ou une spécification de modèle spécifique.
...
Vous pouvez répertorier votre PAP sur plusieurs sites, et certaines revues exigent des articles potentiels pour enregistrer les PAP sur un site spécifique.^[Notez que certaines organisations n'utilisent pas de sites tiers. Par exemple, le [U.S. General Services Administration Office of Evaluation Sciences process](https://oes.gsa.gov/methods/) utilise Github, qui permet de vérifier que le PAP a été créé avant la réalisation de l'analyse.]

- [Registre EGAP](http://egap.org/content/registration)
- [Registre AEA](https://www.socialscienceregistry.org) (pour les ECRs seulement)
