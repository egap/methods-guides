...
Alors que la distribution d'échantillonnage pour l'estimation sans grappe de l'effet du traitement a une erreur standard d'environ 0,52,
...
On peut formaliser l'idée que les grappes fortement dépendantes fournissent moins d'informations que les grappes hautement indépendantes avec le **coefficient de corrélation intra-grappe** (intra-cluster correlation, ICC).
...
où le deuxième terme suit si toutes les grappes ont la même taille ($n_1 \ldots n_J \equiv n$).
...
et l'utiliser pour simuler certaines données à partir d'un design randomisé par grappe simple.
...
Cet ajustement de l'erreur standard IID est communément appelé "erreur standard robuste pour grappe" (robust clustered standard error, RCSE).
...
Plus l'estimation de l'erreur standard est petite, plus les estimations nous semblent précises et plus nous avons de chances de trouver des résultats qui semblent "statistiquement significatifs".
Ceci est problématique : dans ce cas, l'erreur standard IID nous amène à être trop confiants dans nos résultats car elle ignore la corrélation intra-grappe, i.e. dans quelle mesure les différences entre les unités peuvent être attribuées à la grappe dont elles sont membres.
...
Une autre façon d'aborder les problèmes que le clustering introduit dans le calcul des erreurs standard est d'analyser les données au niveau de la grappe.
Dans cette approche, nous prenons des moyennes ou des sommes de résultats au sein des grappes, puis traitons l'étude comme si elle n'avait eu lieu qu'au niveau de la grappe.
...
# Étant donné des grappes de taille égale et sans bloc, ceci est identique à la différence des moyennes au niveau unitaire
...
#4 Pourquoi le clustering peut avoir de l'importance II : différentes tailles de grappe
...
Ainsi, $n_{1k}$ fait référence au nombre d'unités dans la première section de la $k$'ième paire de grappes.
...
#6 Pourquoi le clustering peut avoir de l'importance III : les effets de débordement au sein de la grappe
...
Cependant, il se peut que le résultat d'une unité dépende du statut de traitement $z_j$ d'une autre unité, $j$, à l'intérieur de la même grappe.
...
Lorsque nous menons une expérience randomisée par grappe, nous supposons généralement que le résultat d'une unité n'est pas fonction du statut de traitement des unités avec lesquelles elle partage un grappe,
...
si la grappe est ou non assignée au traitement, ou si quelqu'un se trouve dans la même grappe que d'autres personnes.
...
#7 Que faire à propos des effets de débordement intra-grappes

S'il y a de bonnes raisons de croire que des effets de débordement intra-grappes se produisent,
...
En fin de compte, il faudrait combiner des schémas de randomisation avec grappe et sans grappe afin d'estimer les effets du débordement intra-grappes, $Y_{11} - Y_{01}$ et $Y_{01} - Y_{00}$.
Par conséquent, afin d'interpréter correctement les résultats, les chercheurs doivent être prudents lors de la définition de leur estimande et tenir compte du potentiel de débordement intra-grappe.
...
Dans ce qui suit, nous utilisons l'estimateur des moindres carrés avec une erreur standard robuste pour grappe, afin d'économiser du temps de calcul, mais la même analyse peut être réalisée en utilisant n'importe quel estimateur et statistique de test.
...
# Boucle pour les différentes valeurs de corrélations intra-grappes
