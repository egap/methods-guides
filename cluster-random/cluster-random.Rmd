...
test_H0_and_Htrue <- function(J = J,n = n,treatment_effect = treatment_effect,ICC = ICC) {
  ...
  # Zéro ne devrait pas être trop souvent dans ce CI car l'hypothèse nulle de 0 est fausse ici TODO
...
et comparerons pour une plage de corrélations intra-grappe (intra-cluster correlation, ICC), variant de faible (.01) à élevée (.6).
...
# Boucle pour les différentes valeurs de corrélations intra-cluster
...
On procède de la même manière pour le contrôle de randomisation dans les designs par grappe que précédemment.
Un test valide pour un effet de traitement est un test valide pour le placebo ou l'équilibre des covariables.
...
Ainsi, un test de randomisation avec un petit nombre de grappes peut déclarer trop rapidement une expérience mal randomisée si l'analyste ne connaît pas la méthode d'analyse du taux d'erreur décrite ci-dessus.
...
Pour cette raison, nous recommandons d'utiliser les tests d'hypothèses un par un comme outil exploratoire et d'utiliser les tests omnibus (comme le test T de Hotelling ou un test F ou le test $d^2$ de Hansen et Bowers (2008)), qui peut combiner des informations sur de nombreux tests dépendants en une seule statistique de test pour effectuer directement des tests d'équilibre.
Cependant, ces tests doivent tenir compte du design par grappe : un simple test F sans tenir compte du design par grappe incitera probablement un analyste à déclarer un design déséquilibré et à peut-être accuser le personnel de terrain d'un échec de randomisation.
...
Ainsi, par exemple, si nous avions 10 covariables mesurées au niveau du village et que nous avions un grand nombre de villages, nous pourrions évaluer une hypothèse d'équilibre omnibus en utilisant cet outil basé sur un design à grand échantillon.
...
# Effectuer un test d'équilibre basé sur un design à grand échantillon
...
Une façon d'interpréter ce résultat omnibus est de dire que de tels déséquilibres sur quelques covariables ne modifieraient pas sensiblement les inférences statistiques que nous faisons sur l'effet du traitement tant que ces covariables ne prédisent pas fortement les résultats dans le groupe témoin.
Alternativement, nous pourrions dire que toute grande expérience peut tolérer un déséquilibre aléatoire sur quelques covariables (pas plus de 5% si nous utilisons $\alpha=.05$ comme seuil pour rejeter les hypothèses).
