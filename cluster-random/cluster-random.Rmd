...
Ici, nous montrons que l'effet estimé est le même que nous utilisions une simple différence de moyennes (via la méthode des moindres carrés) ou un modèle à plusieurs niveaux pour ce setup d'essai randomisé par grappe très simplifié.
...
nous pouvons vérifier à quelle fréquence les différentes approches rejettent à tort l'hypothèse nulle stricte d'absence d'effet pour une unité, lorsque nous savons que l'hypothèse nulle d'absence d'effet est vraie.
...
Rappelez-vous que les tests valides auraient des taux d'erreur dans les 2 erreurs type de simulation de 0,95 - cela signifierait qu'une hypothèse nulle correcte serait rejetée pas plus de 5 % du temps. (TODO Recall that, valid tests would have error rates within 2 simulation standard errors of .95 - this would mean that a correct null hypothesis would be rejected no more than 5% of the time.)
...
test_H0_and_Htrue <- function(J = J,n = n,treatment_effect = treatment_effect,ICC = ICC) {
  ...
  # Zéro ne devrait pas être trop souvent dans cet intervalle de confiance car l'hypothèse nulle d'absence d'effet est fausse ici
  ...
  # Zéro devrait être dans cet intervalle de confiance très souvent car l'hypothèse nulle d'absence d'effet est vraie ici
