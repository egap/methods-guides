...
L'éducation augmente-t-elle les revenus ? Les campagnes de santé publique diminuent-elles l'incidence des maladies ? La surveillance internationale peut-elle réduire la corruption gouvernementale ?

Le bruit qui nous préoccupe vient de la complexité du monde.
...
Il serait dommage de se donner toutes les peines et les dépenses nécessaires pour randomiser le traitement, collecter des données sur les groupes de traitement et de contrôle et analyser les résultats, pour que l'effet soit finalement noyé par le bruit de fond.
...
La dernière, le design expérimental, est la seule chose sur laquelle vous avez le pouvoir – utilisez-le !
...
Dans l'ensemble, les chercheurs ne contrôlent pas le bruit de fond, et choisir des variables de résultat à faible bruit est plus facile à dire qu'à faire.
...
Ils peuvent être utiles pour calculer approximativement la taille de l'échantillon dont vous aurez besoin.
...
[^2] : Reproduite de Gerber et Green 2012, page 93
...
* $\mu_t$ est le résultat moyen dans le groupe de traitement. Supposons qu'il soit 65.
* $\mu_c$ est le résultat moyen dans le groupe témoin. Supposons que c'est 60.
* Ensemble, les hypothèses sur μt et c définissent notre hypothèse sur la taille de l'effet du traitement : 65-60= 5.

* $\sigma$ est l'écart type des résultats. C'est ainsi que nous émettons des hypothèses sur le niveau de bruit de notre expérience - l'une des hypothèses que nous émettons est que le sigma est le même pour les groupes de traitement et de contrôle. Supposons $\sigma=20$
* $\alpha$ est notre niveau de signification – la convention dans de nombreuses disciplines est que α doit être égal à 0,05.
$N$ est le nombre total de sujets.
C'est la seule variable qui est sous le contrôle direct du chercheur.
Cette formule suppose que chaque sujet a 50/50 de chances d'être en contrôle.
Supposons que $N=500$.

En utilisant la formule, nous trouvons que sous cet ensemble d'hypothèses, $β = 0,80$, ce qui signifie que nous avons 80 % de chances d'avoir un résultat statistiquement significatif avec ce design.
Cliquez [ici pour une feuille de calcul Google](https://docs.google.com/spreadsheets/d/117R4cqKkhX1MFqPIh7Yg2YzjHykxD7WsSLXqhEbD33I/edit#gid=0){target="_blank"} qui inclut cette formule.
...
Depuis certains points de vue, l'idée d'analyser la puissance statistique n'a aucun sens.
...
Une analyse de puissance qui examine la puissance pour différentes tailles d'étude inclue simplement une plage de valeurs pour N et voit comment β change.

En utilisant la formule de la section 4, vous pouvez voir à quel point la puissance est sensible à toutes les hypothèses : la puissance sera plus élevée si vous supposez que l'effet du traitement sera plus important, ou si vous êtes prêt à accepter un niveau alpha plus élevé, ou si vous avez plus ou moins confiance dans le bruit de vos mesures.[^3]

[^3] : pour un outil supplémentaire de visualisation de puissance en ligne, consultez le [blog R Psychologist] de Kristoffer Magnusson (http://rpsychologist.com/d3/NHST/){target="_blank"}.
...
==
La puissance est une mesure de la fréquence à laquelle, compte tenu des hypothèses, nous obtiendrions des résultats statistiquement significatifs si nous devions mener notre expérience des milliers de fois.
...
stopifnot(all( (possible.ns %% 2)==0 )) ## nécessite un nombre pair de pools expérimentaux
...
for (j in 1:length(possible.ns)){ N <- possible.ns[j] # Choisis la jième valeur pour N
  ...
  for (i in 1:sims){ 
        ...
        Z.sim <- sample(rep(c(0,1),N/2)) ## Faire une assignation aléatoire par tirage au sort sans remise
        ...
        p.value <- summary(fit.sim)$coefficients[2,4] # Extraire p-valeurs
...
Le graphique ci-dessous montre que sous ces hypothèses, vous pouvez vous attendre à obtenir quelques p-valeurs dans la plage de 0.01, mais que 80% seront inférieurs à 0.05.
...
Il existe deux grandes catégories de choix de design qui sont particulièrement importants à cet égard.
...
Contrôlerez-vous les covariables ? Utiliserez-vous un estimateur robuste pour l'erreur standard ?
...
La randomisation en bloc est encore plus puissante : elle garantit qu'un certain nombre au sein d'un sous-groupe seront assignés au traitement.
...
* Exécutez une régression du revenu sur l'âge et le sexe et enregistrez les coefficients, en utilisant des données d'enquête préexistantes (mieux encore : utilisez les données de référence des futurs participants à votre expérience !) l'âge et le sexe d'une manière qui reflète votre groupe de sujets expérimentaux.
...
Voici un graphique qui compare la puissance d'expériences avec et sans contrôle par les attributs d'arrière-plan.
...
Ce graphique montre que pour n'importe quel N, le modèle corrigé des covariables a plus de puissance, à tel point que le modèle non corrigé aurait besoin de 1500 sujets pour réaliser ce que le modèle corrigé des covariables peut faire avec 500 sujets.
...
De nombreuses expériences emploient plusieurs traitements qui sont comparés à la fois les uns aux autres et à un groupe témoin.
...
A) la probabilité qu'au moins un des traitements devienne significatif,
B) la probabilité que tous les traitements deviennent significatifs (par rapport au contrôle)
ou C) la probabilité que les traitements seront classés dans un ordre hypothétique et que ces classements seront statistiquement significatifs.
...
9 Comment penser à la puissance pour les designs par grappe (cluster)
==
...
En règle générale, le clustering diminue votre puissance.
Si vous pouvez éviter de regrouper vos traitements, c'est préférable pour la puissance.
...
Le meilleur scénario pour un design par grappe est lorsque le cluster dans lequel se trouve un sujet fournit très peu d'informations sur leurs résultats.
Supposons que les sujets soient assignés au hasard à des groupes - le groupe n'aiderait pas du tout à prédire les résultats.
Si le cluster n'est pas prédictif du résultat, alors nous n'avons pas perdu trop de puissance à cause du clustering.

Là où le clustering cause vraiment des problèmes, c'est lorsqu'il existe une relation forte entre le cluster et le résultat.
...
Le regroupement peut réduire la taille effective de votre échantillon du nombre total d'individus au nombre total de grappes.

Il existe des formules qui peuvent vous aider à comprendre les conséquences du clustering — voir Gelman/Hill page 447-449 pour une discussion approfondie.
Bien que ces formules puissent être utiles, elles peuvent également être assez lourdes à utiliser.
L'idée principale est cependant simple : vous obtenez généralement plus de puissance en augmentant le nombre de clusters qu'en augmentant le nombre de sujets au sein des clusters.
Mieux vaut avoir 100 groupes avec 10 sujets dans chacun que 10 groupes avec 100 sujets dans chacun.

Encore une fois, une approche plus flexible de l'analyse de puissance lorsqu'il s'agit de clusters est la simulation.
Voir la page de simulations [ici TODO], ou lisez Gelman/Hill page 450-453 pour une autre approche de simulation.
